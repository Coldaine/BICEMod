...
NDiplomacy = {
	...
	BASE_IMPROVE_RELATION_SAME_IDEOLOGY_GROUP_MAINTAIN_COST = 0.1, 			-- Political power cost each update when boosting relations with nation of same ideology
	BASE_IMPROVE_RELATION_DIFFERENT_IDEOLOGY_GROUP_MAINTAIN_COST = 0.2,    	-- Political power cost each update when boosting relations with nation of different ideology
	...
	MAX_OPINION_FROM_VOLUNTEERS = 45,				-- Opinion bonus per one sent volunteer division
	...
	LICENSE_ACCEPTANCE_OPINION_FACTOR = 1,			-- Opinion modifier for acceptance of license production requests.
	...
	MARKET_ACCESS_ACCEPTANCE_OPINION = 0.5,                             -- Acceptance factor for opinion
	...
	MARKET_ACCESS_ACCEPTANCE_TRADE_INFLUENCE = 0.5,                    -- Acceptance factor for trade influence (adjusted from base value)
...
NCountry = {
	...
	CONVOY_RANGE_FACTOR = 0.45,                        			-- how much range affects convoy need
	CONVOY_LENDLEASE_RANGE_FACTOR = 0.45,				-- How much range affects convoy need for lend lease
	CONVOY_INTERNATIONAL_MARKET_RANGE_FACTOR = 0.45,	-- How much range affects convoy need for international market
	...
	COMPLIANCE_PER_COLLABORATION = 0.5,					--each percent of collaboration will be converted to this compliance at capitulation
...
NMilitary = {
	...
	LAND_AIR_COMBAT_ORG_DAMAGE_MODIFIER = 0.015,    	-- global damage modifier
	...
	PLAN_EXECUTE_SUPPLY_CHECK = { 1.0, 0.01, 0.0, 1.0, 0.0 }, -- for each execution mode how careful should we be with supply (1.0 means full required supply available, zero is no limit).
	...
	ARMOR_VS_AVERAGE = 0.30,			            -- how to weight in highest armor vs the division average
...
NAir = {
	...
	SUPPLY_NEED_FACTOR = 1.2, 							-- multiplies supply usage
	...
	DISRUPTION_FACTOR = 12.0,											-- multiplier on disruption damage to scale its effects on planes
	...
	MANPOWER_LOSS_RATIO_PLANE_SHOT = 0.05,	-- The loss ratio of manpower for a shot plane.
...
NNavy = {
	...
	CONVOY_ATTACK_BASE_FACTOR = 0.05,                               -- base % of convoys that get intercepted
	...
	BASE_SPOTTING = 5,												-- base spotting percentage for navy
	...
	SPOTTING_SPEED_MULT_FOR_RUNNING_AWAY = 0.75,					-- task forces that does not want to engage will reduce enemy spotting rate every hour by speed diff mult this ratio
	...
	NAVAL_COMBAT_AIR_CARRIER_TARGET_SCORE = 6,
...
NRailwayGun = {
	...
	ATTACK_TO_ENTRENCHMENT_MODIFIER_FACTOR = 0.0,		-- Entrenchment modifier is calculated by multiplying railway gun attack value with this and dividing by 100
	ATTACK_TO_BOMBARDMENT_MODIFIER_FACTOR = 0.0,	-- Bombardment modifier is calculated by multiplying railway gun attack value with this and dividing by 100
...
NTrade = {
	DISTANCE_TRADE_FACTOR = -0.01,				-- Trade factor is modified by distance times this
	...
	MONTH_TRADE_FACTOR = 3,						-- Each month a trade gets this much boost to it's trade factor
	MAX_MONTH_TRADE_FACTOR = 75,				-- This is the maximum bonus that can be gained from time
	BASE_TRADE_FACTOR = 75,						-- This is the base trade factor
	PUPPET_MASTER_TRADE_FACTOR = 1000,			-- This is priority for puppets to trade with master
	...
	BASE_LAND_TRADE_RANGE = 250,
	PARTY_SUPPORT_TRADE_FACTOR = 30,			-- Trade factor bonus at the other side having 100 % party popularity for my party
...
NAI = {
	...
	DIPLOMACY_IMPROVE_RELATION_COST_FACTOR = 0.1,-- Desire to boost relations subtracts the cost multiplied by this
	...
	DIPLOMACY_SEND_ATTACHE_COST_FACTOR = 0.0,	-- Desire to send attache substracts the cost multiplied by this
	DIPLOMACY_SEND_ATTACHE_PP_FACTOR = 0.0,	-- Desire to send attache adds total PP multiplied by this
...
-- FACTION DIPLO MODIFIER VALUES
	...
	MIN_POLITICAL_POWER_MONTHLY_GAIN_FOR_IMPROVE_RELATIONS = 1000,	-- If country makes less than this PP per month, they won't improve relations
	...
	OPINION_CUTOFF_FOR_IMPROVE_RELATIONS = -101,	-- AI will never consider improving relations if above this opinion with target.
...
-- BATTLEPLAN ACTIVATION
	...
	UPDATE_SUPPLY_MOTORIZATION_FREQUENCY_HOURS = 180;     -- Check if activating motorization would improve supply situation this often.
	DIVISION_SUPPLY_RATIO_TO_MOTORIZE = 0.92,						-- If supply ratio is less than this, consider motorizing any applicable nearby supply hub
...
-- PRODUCTION LOGIC
	...
	DEFAULT_SUPPLY_TRUCK_BUFFER_RATIO = 1.0,	-- ai will set to truck buffer ratio to this. can be modified by wanted_supply_trucks min_wanted_supply_trucks ai strats
...
-- PASSING THROUGH
	...
	MINES_PLANTING_PLANES_PER_MAX_DESIRE = 450,			-- Amount of air wings request for mines planting when there is max desire for it.
	...
	SCREEN_TASKFORCE_MAX_SHIP_COUNT = 9,			-- optimum screen count for screen taskforces
	...
	NAVAL_MISSION_MINES_PLANTING_NEAR_OWNED = 100000,
	NAVAL_MISSION_MINES_PLANTING_NEAR_CONTROLLED = 0,
	NAVAL_MISSION_MINES_SWEEPING_NEAR_OWNED = 100000,	 -- How likely the AI will do the sweeping missions. The value is scaled by the amount of mines to sweep.
	...
	MAX_SCREEN_TASKFORCES_FOR_CONVOY_DEFENSE_MAX_CONVOY_THREAT = 1000,-- AI will increase screen assignment for escort missions as threat increases
	...
	MAX_SCREEN_FORCES_FOR_INVASION_SUPPORT = 0.15, 			-- max ratio of screens forces to be used in naval invasion missions
...
NOperatives = {
	...
	DEFAULT_OPERATION_TIME_MULTIPLIER = -0.12, 
	...
	COUNTER_INTELLIGENCE_FOREIGN_AGENT_FACTOR = 0.33,			-- Multiplier to the counter intelligence provided by foreign (ally) operatives
	...
	ROOT_OUT_RESISTANCE_STACKING_FACTOR = 0.55,				-- Multiplied to each operative efficiency after the first one
...
NSupply = {
	...
	SUPPLY_FLOW_REDUCTION_THRESHOLD = 0.0, -- if supply flow is lower than this, it is not applied
...
NIndustrialOrganisation = {
	...
	ASSIGN_INDUSTRIAL_MANUFACTURER_PP_COST_PER_DAY = 0.00,		-- Cost in Political Power daily generation when one MIO is assigned to a production line
	FUNDS_FOR_SIZE_UP = 1500,					-- Funds needed for a MIO to increment its size and get points to unlock traits
	FUNDS_FOR_SIZE_UP_LEVEL_FACTOR = 1.0, 			-- How much each level mutliplies the funds for size up 
	FUNDS_FOR_SIZE_UP_LEVEL_POW = 2.5, 				-- the power we applie to the mio size when calculating funds to level up. 	
	UNLOCKED_TRAITS_PER_SIZE_UP = 1,			-- Number of points for unlocking traits obtained when the MIO increments its size
	DESIGN_TEAM_CHANGE_XP_COST = 15,				-- Flat cost added to the XP cost of a new equipment design
	...
	FUNDS_FOR_CREATING_EQUIPMENT_VARIANT = 20,		-- Funds added to MIO when a new variant is created with the Design Team assigned to it
	FUNDS_FROM_MANUFACTURER_PER_IC_PER_DAY = 0.12,		-- Funds added to MIO when a manufacturer is attached to a production line. Added every day proportional to IC produced.
	...
	DEFAULT_INITIAL_TASK_CAPACITY = 5,				-- Default start task capacity for each MIO (may be overriden in DB)
